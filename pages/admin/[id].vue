<template>
  ...
  <div class="bg-white p-4 rounded-lg flex-1 flex flex-col shadow-md py-8">
    <p class="text-lg text-center font-light text-white bg-blue-600 rounded-lg p-2 mb-2"> {{ user.first_name || '' }} {{ user.last_name || '' }}
</p>
    <div class="flex flex-col items-center justify-center text-center mt-2">
      <div class="font-light text-sm text-gray-700">Welcome Admin!</div>
...
const fetchNominees = async () => {
  try {
    // 1. Fetch nominees with ratings (includes nominee details)
    const response = await $api.get(`/users/${route.params.id}/ratings`)
    if (response.data.status === 200) {
      nominees.value = response.data.data.map(item => ({
        ...item,
        ...item.nominee, // flatten nominee object
        completion_rate: parseFloat(item.completion_rate) || 0, // 🔥 ensure number
        total_score: parseFloat(item.total_score) || 0,
        overall_score: parseFloat(item.overall_score) || 0,
      }))
    }

    // 2. Fetch overall score rating
    const scoreRes = await $api.get('/dashboard/score-rating')
    if (scoreRes.data.status === 200) {
      overallCompletionRate.value = parseFloat(scoreRes.data.overall_completion_rate) || 0

      // 3. Merge scores into nominees
      scoreRes.data.data.forEach(score => {
        const nominee = nominees.value.find(n => n.id === score.nominee_id)
        if (nominee) {
          nominee.total_score = parseFloat(score.total_score) || 0
          nominee.overall_score = parseFloat(score.overall_score) || 0
          nominee.completion_rate = parseFloat(score.completion_rate) || 0
        }
      })
    }
  } catch (error) {
