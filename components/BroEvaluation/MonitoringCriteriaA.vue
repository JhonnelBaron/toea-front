...
<div class="flex flex-row bg-blue-950 text-white p-4 rounded-t-md justify-between items-center"
  :class="[
    props.form[`A-${criteria.id}`] &&
    (props.form[`A-${criteria.id}`].score || props.form[`A-${criteria.id}`].score === 0)
      ? 'bg-blue-950'
      : 'bg-red-700'
  ]">
        ...
        <div class="border border-gray-300 rounded-b-md p-4 flex flex-row gap-4">
          ...
          <div class="bg-gray-100 p-4 rounded-md flex flex-col gap-4 w-85">
            ...
            <div v-if="props.form[`A-${criteria.id}`]">
              ...
              <select v-model="props.form[`A-${criteria.id}`].score"  disabled @change="() => { submitScore(criteria); emitUpdate() }"  class="w-full border rounded-md p-2 text-sm border-gray-300">
  ...
  <span class="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap z-50 transition-opacity">
    {{ props.form[`A-${criteria.id}`].attachmentName }}
        ...
        <div v-if="props.form[`A-${criteria.id}`]">
          ...
          <textarea v-model="props.form[`A-${criteria.id}`].remarks" @blur="submitScore(criteria)"   disabled class="w-full border rounded-md p-2 text-sm border-gray-300" rows="3" placeholder="Enter remarks..."></textarea>
        </div>
          </div>
