<template>
  <section class="flex bg-white flex-col mb-4 mr-4 rounded-lg shadow-lg p-6">
    <div class="flex flex-col">
      <span class="text-md font-medium p-2">A. Good Governance Measures</span>

      <!-- Loop over B criterias -->
      <div v-for="criteria in aCriterias" :key="criteria.id" class="mb-6">
  ...
  <p class="text-sm">{{ criteria.description }}</p>
        ...
        <div class="border border-gray-300 rounded-b-md p-4 flex flex-row gap-4">
          ...
          <div class="flex flex-col gap-3 w-2/3 rounded-md p-4">
            ...
            <div>
              ...
              <div class="flex flex-col gap-2 mt-2">
                <label v-for="requirement in criteria.a_requirements" :key="requirement.id" class="flex items-center gap-2">
          ...
          <div class="bg-gray-100 p-4 rounded-md flex flex-col gap-4 w-85">
            ...
            <!-- Dropdown -->
            <div v-if="props.form[`A-${criteria.id}`]">
              <label class="text-sm font-light block mb-1">Select Score:</label>
              <select v-model="props.form[`A-${criteria.id}`].score" @change="submitScore(criteria)"   :disabled="isDone" class="w-full border rounded-md p-2 text-sm border-gray-300">
                <option value="">Choose...</option>
                <option v-for="requirement in criteria.a_requirements" :key="'score-'+requirement.id" :value="requirement.point_value" >
                  {{ requirement.point_value }} - {{ requirement.requirement_description }}
                </option>
              </select>
            </div>
...
<div v-if="props.form[`A-${criteria.id}`]" class="border border-gray-300 rounded-md p-4 bg-white shadow-sm flex flex-col gap-3">

  <!-- Upload + Filename + View button -->
  <div class="flex items-center gap-2 w-full">
    <!-- File input: show only if no file -->
  <label 
    v-if="!props.form[`A-${criteria.id}`].attachmentPath"
  ...
  >
    ...
    <!-- File name -->
    <span v-if="props.form[`A-${criteria.id}`].attachmentName" class="text-sm font-medium text-gray-700 truncate flex-1">
      {{ props.form[`A-${criteria.id}`].attachmentName }}
    </span>

    <!-- View File button -->
<button 
  v-if="props.form[`A-${criteria.id}`].attachmentPath" 
  @click="openFilePopup(props.form[`A-${criteria.id}`].attachmentPath, props.form[`A-${criteria.id}`].attachmentType)" 
  ...
  <span class="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap z-50 transition-opacity">
    {{ props.form[`A-${criteria.id}`].attachmentName }}
        ...
        <div v-if="props.form[`A-${criteria.id}`]">
          ...
          <textarea v-model="props.form[`A-${criteria.id}`].remarks" @blur="submitScore(criteria)"   :disabled="isDone" class="w-full border rounded-md p-2 text-sm border-gray-300" rows="3" placeholder="Enter remarks..."></textarea>
        </div>
          </div>
...
onMounted(async () => {
  try {
    const res = await $api.get('/criteria/a')
    aCriterias.value = res.data.data

    // ✅ initialize form after criterias are loaded
    aCriterias.value.forEach(c => {
      const key = `A-${c.id}`
      if (!props.form[key]) {
        props.form[key] = {
          score: '',
    ...
    scores.forEach(s => {
      if (s.criteria_table === 'a_criterias') {
        const key = `A-${s.criteria_id}`
        if (props.form[key]) {
    ...
    console.error('Failed to fetch A criteria:', err)
  }
})

function handleFileUpload(event, criteriaId) {
  const key = `A-${criteriaId}`
...
async function submitScore(criteria) {
  const key = `A-${criteria.id}`
  const fd = new FormData()
  fd.append('nominee_id', route.params.id)
  fd.append('criteria_table', 'a_criterias')
