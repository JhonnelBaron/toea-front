          ...
          <div class="bg-gray-100 p-4 rounded-md flex flex-col gap-4 w-85">
            ...
            <div>
              ...
              <select class="w-full border rounded-md p-2 text-sm border-gray-300">
                ...
                <option v-for="requirement in criteria.c_requirements" :key="'score-'+requirement.id">
                  {{ requirement.point_value }} - {{ requirement.requirement_description }}
                </option>
              </select>
            </div>
...
onMounted(async () => {
  try {
    const res = await $api.get('/criteria/c') // no need for Authorization header, cookie is sent automatically
