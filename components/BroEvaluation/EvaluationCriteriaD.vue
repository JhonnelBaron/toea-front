...
<div v-if="props.form[`D-${criteria.id}`]" class="border border-gray-300 rounded-md p-4 bg-white shadow-sm flex flex-col gap-3">
  ...
  <label 
    v-if="!props.form[`D-${criteria.id}`].attachmentPath"
  ...
  >
    ...
    <!-- File name -->
    <span v-if="props.form[`D-${criteria.id}`].attachmentName" class="text-sm font-medium text-gray-700 truncate flex-1">
      {{ props.form[`D-${criteria.id}`].attachmentName }}
    </span>

    <!-- View File button -->
<button 
  v-if="props.form[`D-${criteria.id}`].attachmentPath" 
  @click="openFilePopup(props.form[`D-${criteria.id}`].attachmentPath, props.form[`D-${criteria.id}`].attachmentType)" 
  ...
  <span class="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap z-50 transition-opacity">
    {{ props.form[`D-${criteria.id}`].attachmentName }}
        ...
        <div v-if="props.form[`D-${criteria.id}`]">
          ...
          <textarea v-model="props.form[`D-${criteria.id}`].remarks" @blur="submitScore(criteria)" :disabled="isDone" class="w-full border rounded-md p-2 text-sm border-gray-300" rows="3" placeholder="Enter remarks..."></textarea>
        </div>
          </div>
